...
## 社区贡献
- [[vLLM]]：Lead Maintainer / Project Lead，负责 engine core、distributed、disaggregated serving、KV Connector 与 observability。
- [[llm-d]]：PD-Disaggregation SIG Lead；同时处于项目创始/核心领导网络，连接 vLLM engine 与 Kubernetes-native distributed serving。

## 人物关系
- [[Tyler Michael Smith]]：**前 Neural Magic、现 Red Hat 同事 + vLLM 共同维护者 + llm-d PD-Disaggregation SIG 共同负责人**。至少在 2025-02，两人共同被 Red Hat 列为 DeepSeek MLA / FP8 优化贡献者；截至 2026-09 又共同负责 llm-d prefill/decode separation、跨实例通信与 disaggregated serving。这里有 SIG 共同负责人和共同工程产出的直接证据，不是从同公司推断。
- [[community/llm-d/llm-d/Carlos Costa|Carlos Costa]]：**llm-d 跨公司项目领导协作；2025–至今**。Robert 属 Red Hat、Carlos 属 IBM，从项目 2025 发布到多个主要版本持续共同署名 llm-d 官方 release，合作上下文是 distributed inference / well-lit paths；该关系记录为 open-source-collaboration，不误标成 paper-coauthor。
- [[community/llm-d/llm-d/Clayton Coleman|Clayton Coleman]]：**llm-d founding / release 关系候选；2025–至少 2026 可核验**。分别来自 Red Hat 与 Google；多次共同出现在官方 release，但本轮不把“共同发布”单独升级成 typed strong edge，留作下一轮进一步核验。
...
- [[Nick Hill]]：**vLLM 开源共同维护者，后分属不同公司**。两人在 2025 DeepSeek-R1 优化中被 Red Hat 官方共同署名，且在 vLLM distributed / KV Connector 维护边界持续交叉；截至 2026-09 Nick 属 [[Inferact]]、Robert 属 [[Red Hat]]。
