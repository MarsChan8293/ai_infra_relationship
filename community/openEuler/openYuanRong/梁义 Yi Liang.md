---
type: person
name: 梁义
english_name: Yi Liang
aliases: [梁义, Yi Liang, liangyi1234]
current_affiliations: ["华为"]
schools:
  - "浙江大学"
projects: ["openYuanRong"]
roles: [openYuanRong Maintainer, 华为通用Serverless首席专家, 华为元戎首席架构师]
areas: [distributed-systems, serverless, ai-infrastructure, scheduling, reinforcement-learning]
last_verified: "2026-09"
---
# 梁义（Yi Liang）

openYuanRong 核心 Maintainer，也是华为通用 Serverless 首席专家 / 元戎首席架构师。公开会议资料显示其博士毕业于浙江大学计算机学院，长期从事分布式系统、实时计算、在线机器学习、Serverless 与 AI Infra。

## openYuanRong
- openEuler `sig-YuanRong` 当前两位 Maintainer 之一，AtomGit ID `liangyi1234`。
- SIGCOMM 2024 YuanRong 论文作者之一；该论文作者单位为 Huawei Technologies。
- 2026 openYuanRong Meetup 以 Maintainer 身份介绍项目定位、架构与社区方向。

## 图谱关系
- [[community/openEuler/openYuanRong/罗站城 Zhancheng Luo|罗站城（Zhancheng Luo）]]：共同担任 openYuanRong / sig-YuanRong Maintainer；只记录公开治理协作，不扩展为私人关系。
- [[community/openEuler/openYuanRong/openYuanRong|openYuanRong]]：核心治理与架构人物。


## 学校关联
- [[university/浙江大学/浙江大学|浙江大学]]：已存在可核验的教育、任职或访问研究关联；具体阶段以正文与 Sources 为准。

## Sources
- https://www.openeuler.org/zh/sig/sig-YuanRong
- https://www.openeuler.org/zh/news/20260728-openYuanrong%20Meetup/20260728-openYuanrong%20Meetup.html
- https://www.openeuler.org/zh/interaction/summit-list/summit2025/
- https://doi.org/10.1145/3651890.3672216
